...
// *** Suica利用履歴
// 端末種
int suica_value_type(suica_value_t *value) {
	return value->type;
}
...
// 入場駅（線区コード、駅順コード）
int suica_value_in_station(suica_value_t *value) {
	return bytes_to_int(value->in_station, sizeof(value->in_station));
}

// 出場駅（線区コード、駅順コード）
int suica_value_out_station(suica_value_t *value) {
	return bytes_to_int(value->out_station, sizeof(value->out_station));
}

// 残額（リトルエンディアン）
int suica_value_balance(suica_value_t *value) {
